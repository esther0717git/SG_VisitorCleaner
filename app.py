...
# --- Apply manual company name corrections (case-insensitive) ---
    # First, explicitly force Sea / Sea Group → Sea Limited
    sea_mask = (
        df["Company Full Name"]
          .astype(str)
          .str.strip()
    ...
    df[wpcol] = df[wpcol].apply(normalize_date_cell)
...
def generate_visitor_only(df: pd.DataFrame, uploaded_file) -> BytesIO:
    ...
    for r in range(2, ws.max_row + 1):
        ...
        mobile_str = "" if mobile_cell is None else str(mobile_cell).strip().lower()

        if mobile_str in ("", "na", "nan", "nil", "00000000"):
